...
static std::array<float, MAX_CLIENT_ID + 1> lastPlayerHealth;
...
void AddDamageEntry(DamageList* damageList, ushort subObjId, float& newHitPoints, enum DamageEntry::SubObjFate fate)
{
	...
	if (iDmgTo && subObjId == 1)
	{
		...
		{
			float hpLost = lastPlayerHealth[iDmgTo] - newHitPoints;
			damageArray[inflictor][iDmgTo] += hpLost;
		}
		lastPlayerHealth[iDmgTo] = newHitPoints;
	}
}
...
void SendDeathMessage(const std::wstring& message, uint system, uint clientVictim, uint clientKiller)
{
	...
	if (clientVictim && clientKiller)
	{
		...
		greatestDamageMessage = ReplaceStr(greatestDamageMessage, L"{2}", stows(itos(static_cast<uint>(ceil((greatestDamageDealt / totalDamageTaken) * 100)))));
